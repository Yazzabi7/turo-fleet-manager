...
const Vehicles = () => {
  ...
  const fetchVehicles = async () => {
    try {
      if (navigator.onLine) {
        ...
        // Cache les véhicules pour le mode hors ligne
        for (const vehicle of fetchedVehicles) {
          await offlineSync.saveVehicle(vehicle);
        }
      } else {
        // Utilise les données en cache si hors ligne
        const cachedVehicles = await offlineSync.getVehicles();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedVehicle) {
        await handleUpdate(selectedVehicle.id, formData);
      } else {
  ...
  const handleUpdate = async (vehicleId: number, data: any) => {
    try {
      if (navigator.onLine) {
        const response = await axios.put(`/api/vehicles/${vehicleId}`, data);
        fetchVehicles();
      } else {
        // Stocke l'action pour la synchronisation ultérieure
        await offlineSync.addPendingAction({
        ...
        const updatedVehicle = { ...vehicles.find(v => v.id === vehicleId), ...data };
        await offlineSync.saveVehicle(updatedVehicle);
        fetchVehicles();
      }
    } catch (error) {
  ...
  const handleDelete = async (vehicleId: number) => {
    ...
    try {
      if (navigator.onLine) {
        ...
        // Supprime localement
        await offlineSync.saveVehicle({ id: vehicleId, deleted: true });
        fetchVehicles();
      }
    } catch (error) {
